...
function checkExistingName(index) {
    if (index === 0)
        return parsedTodos.filter((todo) => todo.name.toLowerCase() === value1.toLowerCase()).length > 0;
    return parsedTodos.filter((todo) => todo.name.toLowerCase() === value2.toLowerCase()).length > 0;
}
...
function addItem() {
    ...
    if (checkExistingName(0)) {
        console.log("This item name already exists, please specify another.")
        return;
    }
